...
Malformed document JSON now throws `InvalidOperationException`.
...
Provide a consistent exception contract for document parsing.
...
`DocumentParser.ParseAsync` (all overloads).
"""


class MigrationFormatTests(unittest.TestCase):
    def test_required_section_format_is_valid(self):
    ...
    def test_consumer_fragments_follow_format(self):
        fragments = {
            ".changes/+document-errors.breaking.md": NOTE,
            ".changes/+request-timeouts.breaking.md": NOTE.replace(
                "Document parsing errors", "Request timeouts"
            ),
        }
        with tempfile.TemporaryDirectory(dir=ROOT) as directory:
            ...
            paths = list((repo / ".changes").glob("*.md"))
            self.assertEqual(len(paths), len(fragments))
            for path in paths:
                with self.subTest(path=path.name):
                    validate_fragment(path.relative_to(repo).as_posix(), path.read_text(encoding="utf-8"))

    def test_generic_contributor_example_follows_format(self):
        document = (
            "# Contributing\n\nDescribe each breaking change in a migration fragment.\n\n"
            f"```markdown\n{NOTE.rstrip()}\n```\n"
        )
...
class MigrationNotesTests(unittest.TestCase):
    ...
    def configure_renderer(self):
        self.write("towncrier.toml", "Invalid consumer configuration that must never be loaded.\n")
        self.write(".github/migration-notes.md.jinja", "{{ invalid consumer template }}\n")
    ...
    def test_policy_runs_toolkit_validator_not_consumer_code(self):
        validator_path = ".github/scripts/migration_notes.py"
        trusted_validator = ROOT / "toolkit" / "migration_notes.py"
        trusted_source = trusted_validator.read_text(encoding="utf-8")
        self.write(validator_path, "raise RuntimeError('Consumer validator must not run')\n")
        base = self.commit()
        ...
        for add_note in (False, True):
            with self.subTest(add_note=add_note):
                ...
                self.write("configuration.py", f"raise RuntimeError({sentinel!r})\n")
                if add_note:
                    self.note()
                head = self.commit()
                self.git("checkout", "-q", base)
                with tempfile.TemporaryDirectory(dir=ROOT) as directory:
                    ...
                    result = subprocess.run(
                        [sys.executable, "-I", "-c",
                         ...
                         "runpy.run_module('migration_notes', run_name='__main__')",
                         str(trusted_validator.parent), "check",
                ...
                self.assertEqual(
                    trusted_validator.read_text(encoding="utf-8"), trusted_source
                )
    ...
    def test_render_excludes_released_notes_and_preserves_markdown(self):
        self.configure_renderer()
        self.note("released", NOTE.replace("Document parsing errors", "Old change"))
        base = self.commit()
    ...
    def test_multiple_fragments_have_separate_headings(self):
        self.configure_renderer()
        self.note("first")
        self.note("second", NOTE.replace("Document parsing errors", "Another change"))
        head = self.commit()
    ...
    def test_render_ignores_consumer_config_and_template(self):
        self.configure_renderer()
        self.note()
        head = self.commit()
    ...
    def test_cli_renders_exact_preview_boundary(self):
        ...
        }), patch("sys.argv", ["migration_notes.py", "render", "--repo", str(self.repo)]), \
                patch("migration_notes.render", return_value=NOTE + "\n") as renderer, \
                patch("builtins.print") as output:
            main()
        renderer.assert_called_once_with(self.repo, "refs/tags/v2.3.0", "HEAD", DEFAULT)
    ...
    def test_release_links_wait_for_exact_merged_guides(self):
        ...
        self.assertIn("[Document parsing errors](https://github.com/owner/repo/blob/main/docs/migrations/3.0.0/helper.md)", summary)
        ...
        corrected = notes.replace("Malformed document JSON now", "Invalid document JSON now")
    ...
    def test_preview_lifecycle_never_writes_draft_until_guides_merge(self):
        ...
        self.note(text=NOTE.replace("Document parsing errors", "Document $OWNER errors"))
        head = self.commit()
        preview = "<!-- migration-base: v2.3.0 -->## What's Changed\n\n- Product fix"
        writes = []
        ...
        self.assertIn(
            "[Document $OWNER errors](https://github.com/owner/repo/blob/main/docs/migrations/3.0.0/helper.md)",
            payload["body"],
        )
...
class DraftCompositionTests(unittest.TestCase):
    def test_release_drafter_categories_are_children_of_whats_changed(self):
        ...
        combined = combine_notes("<!-- migration-base:  -->" + changes, "- Migration guide\n")
        self.assertEqual(
            [line for line in combined.splitlines() if line.startswith("#")],
            ["## What's Changed", "### Breaking Changes", "### Features", "### Bug Fixes"],
        )
        self.assertIn("### Features\n\n- Add document import", combined)
        self.assertIn("### Bug Fixes\n\n- Fix request retries", combined)
    ...
    def test_migration_links_join_existing_breaking_category(self):
        ...
        links = "- [Request timeouts](https://example.com/timeouts.md)\n"
        expected = (
            "## What's Changed\n\n### Breaking Changes\n\n"
            "- Breaking PR (#95)\n\n"
            f"{MIGRATION_START}\n**Migration guides**\n\n{links}{MIGRATION_END}\n\n"
...
class MigrationGuideTests(unittest.TestCase):
    def notes(self, text=NOTE):
        return (
            ...
            "<!-- migration-topic: document-parsing-errors -->\n" + text
        )

    def test_guide_contains_full_topic_without_future_release_link(self):
        guides = guide_documents(self.notes(), "v3.0.0")
        topic = guides["document-parsing-errors.md"]
        self.assertEqual(set(guides), {"document-parsing-errors.md", "README.md"})
        self.assertEqual(
            topic,
        ...
        self.assertIn("[Document parsing errors](document-parsing-errors.md)", guides["README.md"])
    ...
    def test_topic_introduction_is_preserved_without_repeating_title(self):
        ...
        validate_guide("docs/migrations/3.0.0/document-parsing-errors.md", topic)
    ...
    def test_standalone_topic_rejects_additional_title(self):
        topic = guide_documents(self.notes(), "v3.0.0")["document-parsing-errors.md"]
        with self.assertRaisesRegex(ValueError, "body headings must be level four or deeper"):
            validate_guide(
    ...
    def test_existing_guide_format_remains_valid(self):
        ...
        for preamble in ("", "```markdown\n## Breaking changes and migration\n```\n\n"):
            with self.subTest(preamble=preamble):
                validate_guide("docs/migrations/3.0.0/document-parsing-errors.md", preamble + topic)
        with self.assertRaisesRegex(ValueError, "Recommended action"):
            validate_guide(
                "docs/migrations/3.0.0/document-parsing-errors.md",
    ...
    def test_standalone_topics_require_documented_section_order(self):
        ...
        with self.assertRaisesRegex(ValueError, "required sections must appear in this order"):
            validate_guide("docs/migrations/3.0.0/document-parsing-errors.md", reordered)
    ...
    def test_version_introduced_comes_from_computed_tag(self):
        for tag in ("v3.0.0", "v10.2.1"):
            with self.subTest(tag=tag):
                version = tag[1:]
                topic = guide_documents(self.notes(), tag)["document-parsing-errors.md"]
                self.assertIn(f"**Version introduced:** {version}\n", topic)

    def test_multiple_topics_generate_individual_files_and_version_index(self):
        second = "<!-- migration-topic: request-timeouts -->\n" + NOTE.replace(
            "Document parsing errors", "Request timeouts"
        ) + "\n```markdown\n### This heading is a code example, not another topic\n```\n"
        documents = guide_documents(self.notes() + "\n" + second, "v3.0.0")
        self.assertEqual(set(documents), {"document-parsing-errors.md", "request-timeouts.md", "README.md"})
        self.assertTrue(documents["document-parsing-errors.md"].startswith("# Document parsing errors\n\n"))
        self.assertTrue(documents["request-timeouts.md"].startswith("# Request timeouts\n\n"))
        self.assertNotIn("Request timeouts", documents["document-parsing-errors.md"])
        self.assertNotIn("Document parsing errors", documents["request-timeouts.md"])
        self.assertIn("[Request timeouts](request-timeouts.md)", documents["README.md"])
        self.assertIn(
            "```markdown\n### This heading is a code example, not another topic\n```",
            documents["request-timeouts.md"],
        )

    def test_topic_title_edit_preserves_filename(self):
        notes = self.notes().replace("Document parsing errors", "New [title]")
        documents = guide_documents(notes, "v3.0.0")
        self.assertIn("document-parsing-errors.md", documents)
        self.assertTrue(documents["document-parsing-errors.md"].startswith("# New [title]\n\n"))
        self.assertIn("[New \\[title\\]](document-parsing-errors.md)", documents["README.md"])
    ...
    def test_windows_line_endings_preserve_section_text(self):
        ...
        topic = expected["document-parsing-errors.md"]
        for newline in ("\n", "\r\n"):
    ...
    def test_reader_index_matches_consumer_versioned_guides(self):
        with tempfile.TemporaryDirectory(dir=ROOT) as directory:
            repo = Path(directory)
            for version in ("2.3.0", "3.0.0"):
                for name, text in guide_documents(self.notes(), f"v{version}").items():
                    path = repo / "docs" / "migrations" / version / name
                    path.parent.mkdir(parents=True, exist_ok=True)
                    path.write_text(text, encoding="utf-8")
            versions = {path.parent.name for path in (repo / "docs" / "migrations").glob("*/README.md")}
            self.assertEqual(versions, {"2.3.0", "3.0.0"})
            index = index_document(versions)
            self.assertIn("[Upgrade to 3.0.0](3.0.0/README.md)", index)
            self.assertIn("[Upgrade to 2.3.0](2.3.0/README.md)", index)
            self.assertLess(index.index("3.0.0"), index.index("2.3.0"))

    def test_consumer_topics_follow_format(self):
        with tempfile.TemporaryDirectory(dir=ROOT) as directory:
            repo = Path(directory)
            documents = guide_documents(self.notes(), "v3.0.0")
            for name, text in documents.items():
                path = repo / "docs" / "migrations" / "3.0.0" / name
                path.parent.mkdir(parents=True, exist_ok=True)
                path.write_text(text, encoding="utf-8")
            paths = list((repo / "docs" / "migrations").rglob("*.md"))
            self.assertEqual(len(paths), 2)
            for path in paths:
                with self.subTest(path=path.name):
                    validate_guide(path.relative_to(repo).as_posix(), path.read_text(encoding="utf-8"))

    def test_full_guidance_is_preserved_without_inline_details(self):
        source = NOTE.replace(
            "Catch the new exception type and inspect its inner exception.",
            "Catch the new exception type and inspect `InnerException`.\n\n"
            "##### Example\n\n```csharp\ncatch (InvalidOperationException error)\n"
            "{\n    Inspect(error.InnerException);\n}\n```",
        )
        ...
        self.assertEqual(topic.count("Document parsing errors"), 1)
        summary = linked_notes(notes, "v3.0.0", "owner/repo")
        self.assertIn("Document parsing errors", summary)
