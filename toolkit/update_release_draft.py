...
def combine_notes(preview: str, migrations: str, config: Config = DEFAULT) -> str:
    ...
    breaking = [index for index, level, title in headings
                if level == 3 and title == config.categories.breaking]
    if (headings[:1] != [(0, 2, "What's Changed")] or
        ...
        raise ValueError(
            ...
            f"'## What's Changed' and at most one '### {config.categories.breaking}' section."
        )
    ...
    if breaking:
        ...
        insertion = 1
        block = f"### {config.categories.breaking}\n\n" + block
...
def verify_main(repo: Path, commit: str, config: Config = DEFAULT) -> None:
    remote = git(repo, "ls-remote", "origin", f"refs/heads/{config.default_branch}").split()
    if len(remote) != 2 or remote[0] != commit:
        raise ValueError(f"{config.default_branch} changed during generation; rerun Release Drafter.")
...
def process_preview(
    ...
    documents = plan_guides(repo, commit, notes, tag, snapshot, config)
    body = combine_notes(preview, linked_notes(notes, tag, repository, config), config)
    ready = guides_ready(repo, commit, documents, config)
    if prepare:
        verify_main(repo, commit, config)
        check_releases(endpoint, snapshot, name, tag)
        write_guides(repo, commit, documents, config)
        return ready
    if not ready:
        raise ValueError(
