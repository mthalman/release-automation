...
from repository import committed_file, write_target
...
def migration_topics(section: str, config: Config = DEFAULT) -> dict[str, tuple[str, str]]:
    ...
    for index, marker in enumerate(markers):
        ...
        validate_fragment(f"{config.fragment_root}/+{slug}.breaking.md", text, config)
...
def guide_documents(notes: str, tag: str, config: Config = DEFAULT) -> dict[str, str]:
    if not STABLE_TAG.fullmatch(tag):
        raise ValueError(f"Cannot generate migration guides for release tag {tag!r}.")
    if not notes:
        return {}
    version = tag[1:]
    topics = migration_topics(notes, config)
    documents = {}
    links = []
...
def linked_notes(notes: str, tag: str, repository: str, config: Config = DEFAULT) -> str:
    if not STABLE_TAG.fullmatch(tag):
    ...
    links = [
        f"- [{link_label(title)}]"
        f"(https://github.com/{repository}/blob/{config.default_branch}/{config.guide_root}/{tag[1:]}/{slug}.md)"
        for slug, (title, _) in migration_topics(notes, config).items()
    ]
    return "\n".join(links) + "\n"
...
def committed_guides(repo: Path, commit: str, config: Config = DEFAULT) -> dict[str, str]:
    names = git(repo, "ls-tree", "-rz", "--name-only", commit, "--", config.guide_root, config.state_path).split("\0")
    documents = {}
    for name in names:
        if name and (name.endswith(".md") or name == config.state_path):
            content = committed_file(repo, commit, name)
            if content is None:
                raise ValueError(f"Missing committed migration guide: {name}.")
            documents[name] = content
    return documents


def plan_guides(repo: Path, commit: str, notes: str, tag: str, releases: list[dict], config: Config = DEFAULT) -> dict[str, str]:
    generated = guide_documents(notes, tag, config)
    documents = committed_guides(repo, commit, config)
    for name, text in documents.items():
        if name != config.state_path:
            validate_guide(name, text, config)
    pending = pending_guide_versions(documents.get(config.state_path))
    published = {
    ...
    for old_version in pending:
        if old_version in published:
            continue
        prefix = f"{config.guide_root}/{old_version}/"
        referenced = any(
            f"/{prefix}" in (release.get("body") or "") for release in releases
        )
    ...
    prefix = f"{config.guide_root}/{version}/"
    ...
    if generated or config.state_path in documents:
        documents[config.state_path] = json.dumps(
            {"pending_versions": sorted(retained + ([version] if generated else []))}, indent=2
        ) + "\n"
    versions = {name.removeprefix(config.guide_root + "/").split("/")[0] for name in documents
                if name.startswith(f"{config.guide_root}/") and name.count("/") == config.guide_root.count("/") + 2
                and name.endswith("/README.md")}
    documents[f"{config.guide_root}/README.md"] = index_document(versions)
    return documents


def guides_ready(repo: Path, commit: str, documents: dict[str, str], config: Config = DEFAULT) -> bool:
    return committed_guides(repo, commit, config) == documents


def write_guides(repo: Path, commit: str, documents: dict[str, str], config: Config = DEFAULT) -> None:
    removed = committed_guides(repo, commit, config).keys() - documents.keys()
    targets = {name: write_target(repo, name) for name in documents.keys() | removed}
    for name in removed:
        targets[name].unlink()
