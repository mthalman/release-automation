...
The supported interface is two reusable workflows, each with only an optional
`config-path` string. Additional settings belong in the strictly validated
versioned JSON contract, not extra workflow inputs or executable hooks. Changes
...
Toolkit source, asset, or requirement changes need a new immutable payload
commit and refreshed wrapper pins. Follow the
...
with the proposed toolkit and runs an archive of P; it needs that history.
